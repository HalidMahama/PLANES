# This code is based on the original utils script from the https://github.com/michele-segata/plexe-python-demo
# repository. 
import sys
import os
...
def add_vehicle(vid, route, position, lane, speed, cacc_spacing, real_engine=False):
    ...
    if real_engine:
        ...
        set_par(vid, cc.CC_PAR_VEHICLE_MODEL, "audi-r8")
...
def get_dist_to_POI(v, p):
    v_data = get_par(v, cc.PAR_SPEED_AND_ACCELERATION)
    (v, a, u, x1, y1, t) = cc.unpack(v_data)
    px = traci.poi.getPosition(p)[0]
    py = traci.poi.getPosition(p)[1]
