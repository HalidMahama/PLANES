...
class Plane:
    ...
    def topo_contsructor(self):
        ...
        for item in sortd:
            ...
            if current_veh == self._father_vehicle:
                ...
                lane_vehicles = traci.lane.getLastStepVehicleIDs(traci.vehicle.getLaneID(current_veh))[::-1]
    ...
    def move_to_next_best_lane(self, step, flag_n_poi):
        ...
        if traci.simulation.getCurrentTime() > time_flags_found[poi_index][-2] + time_to_pass and flag_found == True:
            vehicles = self._members
            pois = ["exit_POI_0", "exit_POI_1", "exit_POI_2", "exit_POI_3"]
